...
with st.sidebar:
    ...
    # Sidebar — active selection context
    selected_project_name  = st.session_state.get("selected_project_name", "")
    selected_analysis_name = st.session_state.get("selected_analysis_name", "")
    selected_analysis_sid  = st.session_state.get("selected_analysis_sid")

    if selected_project_name or selected_analysis_sid:
        st.markdown("<div style='height:1rem;'></div>", unsafe_allow_html=True)
        st.markdown("""
        <div style="margin:0 1rem;padding:0.75rem;background:#1E3448;border-radius:8px;font-size:0.72rem;color:#A8BFCF;">
            <div style="font-weight:600;color:#E8F4FD;margin-bottom:0.4rem;text-transform:uppercase;letter-spacing:0.06em;">Active Selection</div>
        """, unsafe_allow_html=True)
        if selected_project_name:
            st.markdown(f"""
            <div style="margin-bottom:0.3rem;">
                <span style="color:#4A6580;">Project</span><br>
                <span style="color:#C5D8E8;font-weight:500;">{selected_project_name[:30]}{'…' if len(selected_project_name)>30 else ''}</span>
            </div>""", unsafe_allow_html=True)
        if selected_analysis_sid:
            st.markdown(f"""
            <div>
                <span style="color:#4A6580;">Analysis SID</span><br>
                <span style="color:#C5D8E8;font-weight:500;font-family:monospace;">{selected_analysis_sid}</span>
            </div>""", unsafe_allow_html=True)
...
if page == "🏠  Home":
    ...
    steps = [
        ("1", "📁 Load Projects", "Go to Projects and click Load Projects"),
        ("2", "🔍 Select a Project", "Browse or search, then click Select"),
        ("3", "📋 Load Analyses", "Click Load Analyses to see completed runs"),
        ("4", "📊 Fetch Results", "Go to Results and click Fetch Results"),
        ("5", "⬇ Download", "Download the formatted Excel report"),
    ]
    for col, (num, title, desc) in zip([col_a, col_b, col_c, col_d, col_e], steps):
        with col:
            st.markdown(f"""
            <div class="card" style="text-align:center;padding:1rem;">
                <div style="font-size:1.4rem;font-weight:700;color:#3B82F6;margin-bottom:0.4rem;">{num}</div>
                <div style="font-size:0.8rem;font-weight:600;color:#0D1B2A;margin-bottom:0.3rem;">{title}</div>
                <div style="font-size:0.75rem;color:#6B7E8F;">{desc}</div>
            </div>""", unsafe_allow_html=True)

    if not projects:
        st.info("Start by going to **Projects** and clicking **Load Projects** to connect to Touchstone.")
    elif not st.session_state.get("selected_analysis_sid"):
        st.info("Projects loaded. Go to **Projects** to select an analysis, then visit **Results** to fetch data.")
    else:
        st.success(f"Ready — Analysis SID **{st.session_state['selected_analysis_sid']}** selected. Go to **Results** to fetch data.")
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    load_col, status_col = st.columns([1, 4])
    with load_col:
    ...
    with status_col:
        if projects:
            ...
            st.caption("Click Load Projects to connect to Touchstone and fetch all available projects.")
    ...
    with info_col:
        st.caption(
            f"Showing {start+1}–{min(end, len(filtered))} of {len(filtered):,} projects"
    ...
    with col_info:
        st.markdown(
            f"<div style='text-align:center;font-size:0.8rem;color:#6B7E8F;padding-top:0.5rem;'>"
        ...
        if st.button("📋  Load Analyses"):
            with st.spinner(f"Fetching analyses for {selected_project_name}..."):
                try:
                    ...
                    analyses = get_analyses_for_project(selected_project_sid, selected_project_name)
                    st.session_state["project_analyses"] = analyses
                    st.rerun()
                except Exception as e:
                    st.error(f"Failed: {e}")
    ...
    with ana_status_col:
        if analyses is None:
            st.caption("Click Load Analyses to fetch completed analyses for this project.")
        elif len(analyses) == 0:
            st.warning("No completed analyses found for this project.")
        else:
            st.success(f"✓ {len(analyses)} completed analyses found")

    if analyses and len(analyses) > 0:
        analysis_search = st.text_input(
            "Search analyses",
            placeholder="Filter by name or SID...",
            label_visibility="collapsed",
        ...
        st.caption(
            f"Showing {len(shown)} of {len(filtered_analyses)} analyses"
            + (" (filtered)" if analysis_search else "")
        )
        ...
        if st.session_state.get("selected_analysis_sid"):
            st.success(
                f"✓ Analysis selected — SID {st.session_state['selected_analysis_sid']}. "
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if fetch_clicked:
        ...
        if total > 0:
            st.success(f"✓ {total:,} total records fetched across all datasets")
        else:
            st.warning("No data returned for this analysis. The analysis may still be running or have no results.")
    ...
    <div class="metric-row">
        <div class="metric-card blue">
            ...
            <div class="sub">Event loss table</div>
        </div>
        <div class="metric-card green">
            <div class="label">EP Curves records</div>
            <div class="value">{len(df_ep):,}</div>
            <div class="sub">Annual EP data</div>
        </div>
        <div class="metric-card amber">
            <div class="label">Loss Summary records</div>
            <div class="value">{len(df_summary):,}</div>
            <div class="sub">EP distribution</div>
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    if any(not v.empty for v in results.values()):
        ...
        with info_col:
            st.caption(f"Includes ELT, EP Curves, and Loss Summary sheets · {filename}")
    else:
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if not history:
        st.info("No results pulled yet. Go to Projects to select an analysis, then fetch results.")
        st.stop()

    st.markdown(f"""
    <div class="metric-row" style="grid-template-columns: repeat(2, 1fr);">
        <div class="metric-card blue">
            <div class="label">Analyses pulled</div>
            <div class="value">{len(history)}</div>
            <div class="sub">This session</div>
        </div>
        <div class="metric-card green">
            <div class="label">Total rows fetched</div>
            <div class="value">{sum(r.get("ELT Rows",0) + r.get("EP Rows",0) + r.get("Summary Rows",0) for r in history):,}</div>
            <div class="sub">Across all pulls</div>
        </div>
    </div>""", unsafe_allow_html=True)
